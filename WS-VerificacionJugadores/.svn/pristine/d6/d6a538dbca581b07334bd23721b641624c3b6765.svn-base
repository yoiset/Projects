...
/**
 * Esta clase es invocada por DgojWSSecurityEngine para verificar la firma del mensaje.
 * Nótese que es una reimplementación de "org.apache.ws.security.processor.SignatureProcessor",
 ...
 * debemos hacer están dentro del método principal (que tiene 158 líneas!!). Las variables de instancia son privadas
 * y no tienen getters y setters que permitan sobreescribir la implementación :(
 */
public class DgojSignatureProcessor implements Processor {
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(DgojSignatureProcessor.class);
    private XMLSignatureFactory signatureFactory;
    private KeyInfoFactory keyInfoFactory;
    
    public DgojSignatureProcessor() {
    ...
    /**
     * Este método implementa los cambios descriptos en los comentarios de la clase (ver arriba).
     * Básicamente hemos desactivado un par de verificaciones.
     */
    public List<WSSecurityEngineResult> handleToken(
        ...
        if (keyInfoElement == null) {
            ...
            if (strElements.isEmpty()) {
                ...
                // (Connectis): 
                // Quitamos validación del certificado porque requiere un almacén
                /** 
    ...
    private XMLSignature verifyXMLSignature(
        ...
        try {
            ...
            // (Connectis): 
            // Quitamos el chequeo porque la firma no cumple con eso y si no lanza
            // un error: "Any SIG_REFERENCE MUST contain a SIG_TRANSFORMS child element"
            /**
            if (data.getWssConfig().isWsiBSPCompliant()) {
