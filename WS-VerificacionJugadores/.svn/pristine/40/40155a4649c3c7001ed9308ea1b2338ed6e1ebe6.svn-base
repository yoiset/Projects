...
public class ClienteSCSPService {
	...
	/**
	 * Inicializa el servicio con parámetros que residen en la base de datos. 
	 * Como estos parámetros pueden cambiar en tiempo de ejecución, debemos 
	 * re-asignarlos en cada invocación. 
	 * Nótese que no es una gran penalización porque en realidad los parámetros residen
	 * en una caché gestionada por el DAO (y se vuelven a leer desde la BD solamente 
	...
	private void setClientTimeOut(Map<String, Object> map) {
		...
		map.put(Message.CONNECTION_TIMEOUT, this.getVerificacionDao().getValorPropiedad(Constantes.CFG_SCSP_CONNECT_TIMEOUT));
		map.put(Message.RECEIVE_TIMEOUT, this.getVerificacionDao().getValorPropiedad(Constantes.CFG_SCSP_RESPONSE_TIMEOUT));
		
	}

	public CodigoEstadoSCSP verificarIdentidad(String nif, String numSoporte, String nombre, String apellido1, String apellido2, Date fechaNacimiento) throws Exception {
		try {
			initServicio();
			PeticionSincrona peticion = this.newPeticionSincrona(nif,numSoporte, nombre, apellido1, apellido2,	fechaNacimiento);
	 ...
	 */
	private PeticionSincrona newPeticionSincrona(String nif, String NumSoporte, String nombre, String apellido1, String apellido2, Date fechaNacimiento) {
		...
		if(NumSoporte!=null && !NumSoporte.isEmpty())
		especificos.getSolicitud().setNumSoporte(NumSoporte);
