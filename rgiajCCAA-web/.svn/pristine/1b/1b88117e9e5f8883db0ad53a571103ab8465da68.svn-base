 ...
 * para buscar los tipos de prohibicion segun la informacion rellenada en el formulario de busqueda.
 * @author connectis
 */
@Controller
@RequestMapping(value="/tipoProhibicion/*")
public class TipoProhibicionAjaxGridController extends AbstractPaginatedGridAjaxController<TipoProhibicionBean, TipoProhibicionQueryBean> {

	/**
	 * Servicio de busqueda de Tipos de Prohibicion
	 */
	@Resource
	 ...
	 * Log de la clase.
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(TipoProhibicionAjaxGridController.class);
	
	/**
	 * Peticiones asincronas para la busqueda de tipos de prohibicion a partir del formulario de busqueda.
	 * @param request peticion
	 * @param response respuesta
	 * @return Json con la respuesta a la peticion
	 */
	@ResponseBody
	@RequestMapping(value="/tipoProhibicion/searchTipoProhibicion")
	public String searchTipoProhibicion(HttpServletRequest request, HttpServletResponse response) {		
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="/tipoProhibicion/deleteTipoProhibicion")
	public String deleteTipoProhibicion(HttpServletRequest request, HttpServletResponse response) {
		String result = null;
		try{
			...
			tipoProhibicionService.bajaTipoProhibicion(Long.valueOf(decId), SecurityAccess.getUserInformation());
	 ...
	 */
	@Override
	protected TipoProhibicionQueryBean toQuery() {
		
		TipoProhibicionQueryBean tipoProhibicionQueryBean = new TipoProhibicionQueryBean();
		
		tipoProhibicionQueryBean.setId(RequestUtil.getLongParameter(this.getRequest(), "id", null));
		tipoProhibicionQueryBean.setCodigo(RequestUtil.getStringParameter(this.getRequest(), "codigo", null));
		tipoProhibicionQueryBean.setDescripcion(RequestUtil.getStringParameter(this.getRequest(), "descripcion", null));
		tipoProhibicionQueryBean.setActivo(RequestUtil.getBooleanParameter(this.getRequest(), "activo", null));
		return tipoProhibicionQueryBean;
	}
	 ...
	 * @param tipoProhibicionQueryBean Objeto con los parametros de buscado obtenidos previamente de la request
	 * @return Objeto de resultado con la informacion de las incidencias.
	 */
	@Override
	protected SearchResult<TipoProhibicionBean> doSearch(TipoProhibicionQueryBean tipoProhibicionQueryBean) {
		// Almaceno los criterios de búsqueda en sesión de usuario
		this.getRequest().getSession().setAttribute("tipoProhibicionQueryBean", tipoProhibicionQueryBean);
		
		TipoProhibicionSearchResult tipoProhibicionResults = this.tipoProhibicionService.getTiposProhibicion(tipoProhibicionQueryBean);
		
		SearchResult<TipoProhibicionBean> result = new SearchResult<TipoProhibicionBean>(tipoProhibicionResults.getResults());
		result.setNumResults(tipoProhibicionResults.getNumResults());
	 ...
	 */
	@Override
	protected String beanId(TipoProhibicionBean bean) {
	 ...
	 */
	@Override
	protected String[] beanToStrings(TipoProhibicionBean bean) {
