...
function defineGrid(id) {
  ...
  grid.addCol({header:'<spring:message code="page.tipoFirma.grid.label.codigo" />', width:180, sorting:"str"});
  grid.addCol({header:'<spring:message code="page.tipoFirma.grid.label.descripcion" />', width:539, sorting:"na"});
  grid.addCol({header:'<spring:message code="page.tipoFirma.grid.label.activo" />', width:70, sorting:"na"});
  grid.addCol({header:'<spring:message code="page.tipoFirma.grid.label.defecto" />', width:70, sorting:"na"});
  return grid;
}

var selectedTipoFirmaId = "";
...
function doReset() {
  	...
  	$.ajax({
	    type: "POST",
	    cache: false,
	    url: contextPath + "/app/tipoFirma/limpiarBusqueda"
	});
}

function doEdit() {
	  if (selectedTipoFirmaId == "") {
	    alert('<spring:message code="page.tipoFirma.alert.selectRegistro" />');
	    return;
	  }

	  document.getElementById("editTipoFirma").href = contextPath + "/app/tipoFirma/startEditTipoFirma?id=" + selectedTipoFirmaId;
	  document.getElementById("editTipoFirma").click();
}
...
function doDelete() {
	...
	if (!confirm('<spring:message code="page.tipoFirma.confirm.deleteRegistro"/>')) {
	  return;
	}
	 ...
	 $.ajax({
		    type: "POST",
		    cache: false,
		    url: contextPath + "/app/tipoFirma/deleteTipoFirma",
		    data: "id=" + selectedTipoFirmaId,
...
String.prototype.ltrim = function() {
	return this.replace(/^\s+/,"").replace(/&nbsp;/g,"");
          ...
          <div id="formulcuad">  
              ...
              <form:form id="searchForm" action="${pageContext.request.contextPath}/app/tipoFirma/searchTipoFirma"
					...
					<div class="breadcrumb">
					  ...
					  <ul class="breadcrumb">
					    ...
					    <li><a href="${pageContext.request.contextPath}/app/tipoFirma/startTipoFirma" class="active"><spring:message code="page.breadcrumb.tipoFirma"/></a></li>
					  </ul>
					...
					<div class="listado">
						<div class="encabezado">
				  			<label for="usuario">  					
								<span class="label">
				 					<spring:message code="page.breadcrumb.tipoFirma"/>										
								</span>
				 			</label>  								
						</div>		
					</div>
                      ...
                      <legend><spring:message code="page.tipoFirma.header.filter"/></legend>
                      
                  	  <label class="linea-medio">
               	  		<span class="label_col1"><spring:message code="page.tipoFirma.form.codigo.label"/></span>
                     	<form:input type="text" id="codigo" name="codigo" path="codigo" 
                       	alt='${msg["page.tipoFirma.form.codigo.title"]}' 
                       	title='${msg["page.tipoFirma.form.codigo.title"]}'></form:input>    
                      </label>
                      
                      <div class="listado-acciones-centrado">
                        <input type="button" id="clear" 
                          alt='${msg["page.tipoFirma.form.clear.title"]}' 
                          title='${msg["page.tipoFirma.form.clear.title"]}' 
                          value='${msg["page.tipoFirma.form.clear.value"]}' 
                          class="medio"
                          onclick="doReset()"/>
                        <input type="button" id="filter" 
                          alt='${msg["page.tipoFirma.form.filter.title"]}' 
                          title='${msg["page.tipoFirma.form.filter.title"]}' 
                          value='${msg["page.tipoFirma.form.filter.value"]}' 
                          class="medio" 
		            ...
		            <div class="mod-pestanas modulo">
		              ...
		              <div class="listado-acciones-derecha">
			            ...
			            <sec:authorize ifAllGranted="MANT-BAS_NUEVA">
		                    <a href="${pageContext.request.contextPath}/app/tipoFirma/startNewTipoFirma" 
				                      title='${msg["page.tipoFirma.actions.new.title"]}' 
				                      class="medio"><spring:message code="page.tipoFirma.actions.new.label"/></a>
						</sec:authorize>
				              
						<sec:authorize ifAllGranted="MANT-BAS_MODIFICAR">
		                    <a id="editTipoFirma" title='${msg["page.tipoFirma.actions.edit.title"]}'  
		                          class="medio" onclick="doEdit()"><spring:message code="page.tipoFirma.actions.edit.label"/></a>   
						</sec:authorize>
						
						<sec:authorize ifAllGranted="MANT-BAS_BORRAR">	
			                <a id="deleteTipoFirma" title='${msg["page.tipoFirma.actions.delete.title"]}' 
	   	                       class="medio" onclick="doDelete()"><spring:message code="page.tipoFirma.actions.delete.label"/></a>
	   	                </sec:authorize>
