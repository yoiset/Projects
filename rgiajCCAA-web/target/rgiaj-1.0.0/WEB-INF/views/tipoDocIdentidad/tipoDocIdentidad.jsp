...
function doOnSelectStateChangedPending(rowID) {
	selectedTipoDocIdentidadId = rowID;
  	updateLinks();
}
...
function defineGrid(id) {
  ...
  grid.addCol({header:'<spring:message code="page.tipoDocIdentidad.grid.label.codigo" />', width:250, sorting:"str"});
  grid.addCol({header:'<spring:message code="page.tipoDocIdentidad.grid.label.descripcion" />', width:539, sorting:"na"});
  grid.addCol({header:'<spring:message code="page.tipoDocIdentidad.grid.label.activo" />', width:70, sorting:"na"});
  
  return grid;
}

var selectedTipoDocIdentidadId = "";
...
function doReset() {
  	...
  	selectedTipoDocIdentidadId = "";
  	updateLinks();
  	document.getElementById("numRegistrosTotales").innerHTML="";
  	$.ajax({
	    type: "POST",
	    cache: false,
	    url: contextPath + "/app/tipoDocIdentidad/limpiarBusqueda"
	});
}

function doEdit() {
	  if (selectedTipoDocIdentidadId == "") {
	    alert('<spring:message code="page.tipoDocIdentidad.alert.selectRegistro" />');
	    return;
	  }

	  document.getElementById("editTipoDocIdentidad").href = contextPath + "/app/tipoDocIdentidad/startEditTipoDocIdentidad?id=" + selectedTipoDocIdentidadId;
	  document.getElementById("editTipoDocIdentidad").click();
}
...
function doDelete() {
	...
	if (!confirm('<spring:message code="page.tipoDocIdentidad.confirm.deleteRegistro"/>')) {
	  return;
	}
	 ...
	 $.ajax({
		    type: "POST",
		    cache: false,
		    url: contextPath + "/app/tipoDocIdentidad/deleteTipoDocIdentidad",
		    data: "id=" + selectedTipoDocIdentidadId,
...
String.prototype.ltrim = function() {
	return this.replace(/^\s+/,"").replace(/&nbsp;/g,"");
          ...
          <div id="formulcuad">  
              ...
              <form:form id="searchForm" action="${pageContext.request.contextPath}/app/tipoDocIdentidad/searchTipoDocIdentidad"
					...
					<div class="breadcrumb">
					  ...
					  <ul class="breadcrumb">
					    ...
					    <li><a href="${pageContext.request.contextPath}/app/tipoDocIdentidad/startTipoDocIdentidad" class="active"><spring:message code="page.breadcrumb.tipoDocIdentidad"/></a></li>
					  </ul>
					...
					<div class="listado">
						<div class="encabezado">
				  			<label for="usuario">  					
								<span class="label">
				 					<spring:message code="page.breadcrumb.tipoDocIdentidad"/>										
								</span>
				 			</label>  								
						</div>		
					</div>
                      ...
                      <legend><spring:message code="page.tipoDocIdentidad.header.filter"/></legend>
                      
                  	  <label class="linea-medio">
               	  		<span class="label_col1"><spring:message code="page.tipoDocIdentidad.form.codigo.label"/></span>
                     	<form:input type="text" id="codigo" name="codigo" path="codigo" 
                       	alt='${msg["page.tipoDocIdentidad.form.codigo.title"]}' 
                       	title='${msg["page.tipoDocIdentidad.form.codigo.title"]}'></form:input>    
                      </label>
                      
                      <div class="listado-acciones-centrado">
                        <input type="button" id="clear" 
                          alt='${msg["page.tipoDocIdentidad.form.clear.title"]}' 
                          title='${msg["page.tipoDocIdentidad.form.clear.title"]}' 
                          value='${msg["page.tipoDocIdentidad.form.clear.value"]}' 
                          class="medio"
                          onclick="doReset()"/>
                        <input type="button" id="filter" 
                          alt='${msg["page.tipoDocIdentidad.form.filter.title"]}' 
                          title='${msg["page.tipoDocIdentidad.form.filter.title"]}' 
                          value='${msg["page.tipoDocIdentidad.form.filter.value"]}' 
                          class="medio" 
		            ...
		            <div class="mod-pestanas modulo">
		              ...
		              <div class="listado-acciones-derecha">
			            ...
			            <sec:authorize ifAllGranted="MANT-BAS_NUEVA">
		                    <a href="${pageContext.request.contextPath}/app/tipoDocIdentidad/startNewTipoDocIdentidad" 
				                      title='${msg["page.tipoDocIdentidad.actions.new.title"]}' 
				                      class="medio"><spring:message code="page.tipoDocIdentidad.actions.new.label"/></a>
						</sec:authorize>
				              
						<sec:authorize ifAllGranted="MANT-BAS_MODIFICAR">
		                    <a id="editTipoDocIdentidad" title='${msg["page.tipoDocIdentidad.actions.edit.title"]}'  
		                          class="medio" onclick="doEdit()"><spring:message code="page.tipoDocIdentidad.actions.edit.label"/></a>   
						</sec:authorize>
						
						<sec:authorize ifAllGranted="MANT-BAS_BORRAR">	
			                <a id="deleteTipoDocIdentidad" title='${msg["page.tipoDocIdentidad.actions.delete.title"]}' 
	   	                       class="medio" onclick="doDelete()"><spring:message code="page.tipoDocIdentidad.actions.delete.label"/></a>
	   	                </sec:authorize>
